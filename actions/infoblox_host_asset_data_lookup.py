# File: infoblox_host_asset_data_lookup.py
#
...
class HostAssetDataLookup(BaseAction):
    """Class to handle host asset data lookup action.

    This action allows filtering and retrieving IPAM hosts by specific criteria
    such as host filters, tag filters, with pagination support.
    """

    def __log_action_start(self):
        """Log the start of the action execution."""
        self._connector.save_progress(consts.EXECUTION_START_MSG.format("Host Asset Data Lookup"))
        ...
        if limit is not None:
            ...
            if limit > consts.MAX_LIMIT:
                return self._action_result.set_status(
                    phantom.APP_ERROR,
                    "Limit parameter must be less than or equal to 1000",
                )
    ...
    def __make_api_call(self):
        """
        Make the API call to retrieve IPAM hosts.
        ...
        host_filter = self._param.get("host_filter")
        if host_filter:
            params["_filter"] = host_filter
        ...
        endpoint = consts.IPAM_HOST_ENDPOINT
    ...
    def __handle_response(self, response):
        """
        Handle the API response and process the IPAM hosts data.
        ...
        self._connector.debug_print("Processing IPAM hosts response")
        ...
        summary = {
            ...
            "offset": offset,
            "limit": limit,
            "host_filter_applied": bool(host_filter),
        ...
        success_message = consts.ACTION_HOST_ASSET_DATA_LOOKUP_SUCCESS_RESPONSE.format(count=total_hosts)

        return self._action_result.set_status(phantom.APP_SUCCESS, success_message)

    def execute(self):
        """
        Execute host asset data lookup action following the modular approach.
        ...
        # Step 3: Make API call
        self._connector.save_progress("Retrieving IPAM hosts")
        ...
        # Step 5: Return results
        self._connector.save_progress("IPAM hosts retrieved successfully")
        return phantom.APP_SUCCESS
